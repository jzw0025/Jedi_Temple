...
load = sio.loadmat("/Users/junchaowei/Desktop/new_segmentation.mat")
volume = load['par1']

points, tri, normal, values = measure.marching_cubes_lewiner(volume, 0.6, step_size=20)

def extractLinks(ntri):
    """
    this function extracting the links from the surface triangle mesh
    tri --- N by 3 ndarray mesh
    """
    edge1 = np.sort(np.array([ntri[:,0], ntri[:,1]]),axis = 0) # this is 2 by N matrix, sorted by the axis 0
    edge2 = np.sort(np.array([ntri[:,0], ntri[:,2]]),axis = 0)
    edge3= np.sort(np.array([ntri[:,1], ntri[:,2]]),axis = 0)
    edge = np.concatenate((edge1,edge2,edge3),axis=1)
    trans_edge = edge.T
    b = np.ascontiguousarray(trans_edge).view(np.dtype((np.void, trans_edge.dtype.itemsize * trans_edge.shape[1])))
    _, idx = np.unique(b, return_index=True)
    return trans_edge[idx]

out = extractLinks(tri)
sx, sy = out.shape
#NodeCoordinates = np.array([[0.0,0.0,0.0],[10.0,0.0,0.0],[10.0,10.0,0.0]])
#ElementNodes = [[0,1],[1,2],[0,2]]

NodeCoordinates = points
ElementNodes =out.tolist()
...
if __name__ == "__main__":
    ...
    EleMat = [1.0 for i in xrange(sx)]
    EleFab = [100 for i in xrange(sx)]
