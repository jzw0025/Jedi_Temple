...
double FEM<dim>::basis_function(unsigned int node, double xi_1, double xi_2){
  ...
  if (node == 0)
    ...
    {value = 1./4.*(1 + xi_1)*(1 + xi_2);}
  eles
    {std::cout<< "unknow local node numbering(beyond0,1,2,3)"<<std::endl;}//EDIT
...
void FEM<dim>::define_boundary_conds(){
  ...
  //EDIT - Define the Dirichlet boundary conditions.
        for (unsigned int i=0; i<nodeLocation.sizes()[0]; ++i)
             {
                  if(t[i][1] == 0.0)
                 { boundary_values[globalNodeIndex] = 300.*(1. + 1./3.*t[i][0]);}
		 if(t[i][1] == 0.08)
                 { boundary_values[globalNodeIndex] = 310.*(1. + 8.*t[i][0]*t[i][0]);}
             }
  ...
  for (;elem!=endc; ++elem){
	...
	for(unsigned int A=0; A<dofs_per_elem; A++){
	  for(unsigned int B=0; B<dofs_per_elem; B++){
		...
		for(unsigned int I=0;I<dim;I++){
		  for(unsigned int J=0;J<dim;J++){
		    Klocal(A,B) += (basis_gradient(A,quad_points[q1],quad_points[q2])[i]*invJacob(i,I)+basis_gradient(A,quad_points[q1],quad_points[q2])[j]*invJacob(j,I))*kappa(I,J)*(basis_gradient(B,quad_points[q1], quad_points[q2])[i]*invJacob(i,J)+basis_gradient(B,quad_points[q1],quad_points[q2])[j]*invJacob(j,J))*detJ*quad_weight[q1]*quad_weight[q2]; 
