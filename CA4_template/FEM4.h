  ...
  for (;elem!=endc; ++elem){
	...
	for(unsigned int q=0; q<num_quad_pts; q++){
	  for(unsigned int i=0; i<dim; i++){
	    for(unsigned int j=0; j<dim; j++){
               Klocal(A,B) += fe_values.shape_grad(A,q)*kappa[i][j]*fe_values.shape_grad(B,q)*fe_values.JxW(q);
	...
	int A = local_dof_indices[i];
	int B = local_dof_indices[j];
	double C = Klocal(i,j);
        double D = Mlocal(i,j);
	K.add(A,B,C);
	M.add(A,B,D);
...
void FEM<dim>::apply_initial_conditions(){
 ...
 std::cout<< "initial D trans: " << D_trans << std::endl;
  ...
  std::cout<< "D current_l2norm: "<<current_l2norm << std::endl;
  l2norm_results.push_back(current_l2norm);
  std::cout<< "initial V trans: " << V_trans << std::endl;
}

//Solve for D_transient, Solve for D_transient, Solve for D_transient, Solve for D_transient , Solve for D_transient
template <int dim>
void FEM<dim>::solve_trans(){
  ...
  for(unsigned int t_step=1; t_step<3001; t_step++){
    ...
    //V_n *= delta_t; // delta_t*(1-alpha)*V_n
    if(t_step==1){
        std::cout<< "V trans: " << V_trans << std::endl;
        std::cout<< "D trans: " << D_trans << std::endl;
        std::cout<< "D tilde: "<<D_tilde << std::endl;
        }
    D_tilde = D_trans; //
    //if(t_step==1){
    //    std::cout<< "D tilde: "<<D_tilde << std::endl;
    ...
    //if(t_step==1){
    //    std::cout<< "V_trans: "<<V_trans << std::endl;
    //    std::cout<< "delta_t*(1.0-alpha): "<<delta_t*(1.0-alpha) << std::endl;
    //    std::cout<< "D tilde: "<<D_tilde << std::endl;
    //}
    //D_tilde = D_trans + delta_t*(1.0-alpha)*  V_trans;
    //EDIT
    ...
    //std::cout<< "sum: " << std::accumulate(V_trans.begin(), V_trans.end(), 0)<< std::endl;
    ...
    if(t_step%100 == 0){
      ...
      double current_l2norm = l2norm();
      std::cout<< "current_l2norm: "<<t_step<<" : "<< current_l2norm<< std::endl;
      l2norm_results.push_back(current_l2norm);
    }
  }
}
